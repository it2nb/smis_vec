...
if($_POST["save_bt"]=="บันทึก")
{
	...
	if(mysql_num_rows($checkperiod_query)>0){
		...
		$query = "Insert Into period(period_year,period_term,period_start,period_end) Values ('$period_year','$period_term','$period_start','$period_end')";
		$Insert_period = mysql_query($query,$conn)or die(mysql_error());
		$member_ID = $_SESSION["userID"];
		$userlogs_des="เพิ่มภาคเรียนที่ ".$period_term." ปีการศึกษา ".($period_year+543)."เริ่ม ".$period_start." สิ้นสุด ".$period_end;
		$query="Insert Into userlogs(member_ID,userlogs_action,userlogs_system,userlogs_des) Values ('$member_ID','Add Period','period_mis','$userlogs_des')";
...
$thisdate = date("Y-m-d");
try {
$spdate = new DateTime($thisdate,new DateTimeZone('Asia/Bangkok'));
} catch (Exception $e) {
    echo $e->getMessage();
    exit(1);
}
...
$(document).ready(function() { 
    ...
    <tr bgcolor="#009999">
      <td align="center" valign="middle"><b>ความคุม</b></td>
        ...
        <div id="admincontent">
        <center>
        <form id="addperiodform" action="addperiod.php" method="post" enctype="multipart/form-data"><table width="80%" border="0" cellspacing="0" cellpadding="0">
  <tr>
