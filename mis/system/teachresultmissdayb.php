...
$(document).ready(function() {
    ...
    <div id="admincontent">
  <center><br />
	<form id="periodsearchform" action="teachresultmissdayb.php" method="post">
		...
		?>
         <table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000">
                <th height="50" colspan="7" bgcolor="#CCCCCC">
          ...
          <tr>
            ...
            <td width="17%" align="center" valign="middle" bgcolor="#FFCC33" class="BlackBold10">ชื่อ - สกุล</td>
            <td align="center" valign="middle" bgcolor="#FFCC33" class="BlackBold10">สาขา</td>
            <td width="20%" align="center" valign="middle" bgcolor="#FFCC33" class="BlackBold10">วิชา/ครูผู้สอน</td>
  ...
  <?php
  	$styear=$check_year-3;
	$query = "Select DISTINCT student.student_ID,student_prefix,student_name,student_ser,student_parphone,class.* From class,teachcheck,student,teachday,teach Where class.class_ID=student.class_ID and student.student_ID=teachcheck.student_ID and teachcheck.teachday_ID=teachday.teachday_ID and teachcheck.teach_ID=teach.teach_ID and teachcheck.teachcheck_week='$thisweek' and teachday_day='$thisday' and teachcheck_result='0' and teach_year='".substr($check_period,2,4)."' and teach_term='".substr($check_period,0,1)."' and student.class_ID like '__2_____' Order By student.class_ID DESC";
	$studentvo_query=mysql_query($query,$conn) or die(mysql_error());	
	$query = "Select DISTINCT student.student_ID,student_prefix,student_name,student_ser,student_parphone,class.* From class,teachcheck,student,teachday,teach Where class.class_ID=student.class_ID and student.student_ID=teachcheck.student_ID and teachcheck.teachday_ID=teachday.teachday_ID and teachcheck.teach_ID=teach.teach_ID and teachcheck.teachcheck_week='$thisweek' and teachday_day='$thisday' and teachcheck_result='0' and teach_year='".substr($check_period,2,4)."' and teach_term='".substr($check_period,0,1)."' and student.class_ID like '__3_____' Order By student.student_ID DESC";
	$studentde_query=mysql_query($query,$conn) or die(mysql_error());	
  	$allvo=0;
  while($student_fetch=mysql_fetch_assoc($studentvo_query))
  {
  ...
  <?php 
	...
	$sumvo_fetch = mysql_fetch_assoc($sumvo_query);
	$sumvo = $sumvo_fetch["sumvo"];
	$percentvo = round(($allvo/$sumvo)*100,2);
  ?><tr bgcolor="#FFFF99">
     <td height="25" colspan="3" align="center" valign="middle"><b>รวม ปวช.</b></td>
     <td height="25" colspan="4" align="center" valign="middle"><b><?php echo "ขาดจำนวน $allvo คน จาก $sumvo คน คิดเป็น $percentvo %";?></b></td>
     </tr>
  
   <?php
   $allde=0;
  while($student_fetch=mysql_fetch_assoc($studentde_query))
  {
	  	$student_show=$check_year-$student_fetch["year"];
	  	$query = "Select personnel_name,personnel_ser From personnel Where personnel_ID='".$student_fetch["personnel_ID"]."'";
	  	$personnel_query = mysql_query($query,$conn)or die(mysql_error());
  ...
  <?php 
	...
	$sumde_fetch = mysql_fetch_assoc($sumde_query);
	$sumde = $sumde_fetch["sumde"];
	$percentde = round(($allde/$sumde)*100,2);
  ?><tr bgcolor="#FFFF99">
     <td height="25" colspan="3" align="center" valign="middle"><b>รวม ปวส.</b></td>
     <td height="25" colspan="4" align="center" valign="middle"><b><?php echo "ขาดจำนวน $allde คน จาก $sumde คน คิดเป็น $percentde %";?></b></td>
     </tr>
   <tr bgcolor="#FFFF99">
     <td height="25" colspan="3" align="center" valign="middle"><b>รวม ทั้งหมด</b></td>
     <td height="25" colspan="4" align="center" valign="middle"><b><?php echo "ขาด ".($allvo+$allde)." คน จาก ".($sumvo+$sumde)." คน คิดเป็น ".(round((($allvo+$allde)/($sumvo+$sumde))*100,2))." %";?></b></td>
     </tr>
</table><?php }?><br />
</center></div>