	...
	$personnel_fetch=mysql_fetch_assoc($personnel_query);
 	while($class_fetch=mysql_fetch_assoc($class_query)){
		...
		for($sl=1;$sl<=ceil(mysql_num_rows($student_query)/30);$sl++){
			$r=1;
		$query="Select * From scoregroup Where scoregroup_name!='finalID".(int)$teach_ID."' and teach_ID='$teach_ID' Order By scoregroup_ID ASC";
		$scoregroup_query=mysql_query($query,$conn)or die(mysql_error());
		while($scoregroup_fetch=mysql_fetch_assoc($scoregroup_query)){
 ...
 ?>
    ...
    <center>
      <div align="left" class="content">
        ...
        <?php echo "รหัสวิชา ".$teach_fetch["subject_ID"]." ชื่อวิชา ".$teach_fetch["subject_name"]."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ทฤษฎี ".$teach_fetch["subject_hourt"]." ปฏิบัติ ".$teach_fetch["subject_hourp"]." หน่วยกิต ".$teach_fetch["subject_unit"];?><br>
        <?php echo "ระดับชั้น ".$class_fetch["area_level"].".".(substr($teach_fetch["teach_year"]+543,2,2)-substr($class_fetch["class_ID"],0,2)+1)."/".(int)substr($class_fetch["class_ID"],6,2)." สาขางาน ".$class_fetch["major_name"]."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ชื่อผู้สอน".$personnel_fetch["personnel_name"]." ".$personnel_fetch["personnel_ser"];?><br>
        สอนวัน
        <?php
  ...
  <tr>
    <th height="120" colspan="3" align="center" valign="middle" class="BlackBold10"><?php echo $scoregroup_fetch["scoregroup_name"];?></th>
    <th width="20" align="left" valign="top"><span class="verticaltext"><?php if($scoredetail_num>=$sdnum)echo mysql_result($scoredetail_query,($sdnum++)-1,"scoredetail_name");?></span></th>
    <th width="20" align="left" valign="top"><span class="verticaltext"><?php if($scoredetail_num>=$sdnum)echo mysql_result($scoredetail_query,($sdnum++)-1,"scoredetail_name");?></span></th>
    <th width="20" align="left" valign="top"><span class="verticaltext"><?php if($scoredetail_num>=$sdnum)echo mysql_result($scoredetail_query,($sdnum++)-1,"scoredetail_name");?></span></th>
    <th width="20" align="left" valign="top"><span class="verticaltext"><?php if($scoredetail_num>=$sdnum)echo mysql_result($scoredetail_query,($sdnum++)-1,"scoredetail_name");?></span></th>
    <th width="20" align="left" valign="top"><span class="verticaltext"><?php if($scoredetail_num>=$sdnum)echo mysql_result($scoredetail_query,($sdnum++)-1,"scoredetail_name");?></span></th>
    ...
    <th width="20" align="left" valign="top"><span class="verticaltext"><?php if($scoredetail_num>=$sdnum)echo mysql_result($scoredetail_query,($sdnum++)-1,"scoredetail_name");?></span></th>
    <th width="20" align="left" valign="top"><span class="verticaltext"><?php if($scoredetail_num>=$sdnum)echo mysql_result($scoredetail_query,($sdnum++)-1,"scoredetail_name");?></span></th>
    <th width="20" align="left" valign="top"><span class="verticaltext">รวม</span></th>
    <th width="20" align="left" valign="top"><span class="verticaltext"><b>รวมคะแนน</b></span></th>
  </tr>
  <tr>
  ...
  while($r<=30&&$student_fetch=mysql_fetch_assoc($student_query))
  {
	  if($student_fetch["student_endstatus"]==0){
		...
		$query="Select * From stdscore,scoredetail Where stdscore.scoredetail_ID=scoredetail.scoredetail_ID and student_ID='$student_ID' and scoregroup_ID='".$scoregroup_fetch["scoregroup_ID"]."'";
